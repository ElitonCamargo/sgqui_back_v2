...
export const createAutoProdutosSchema = z.object({
  projeto_id: z.coerce.number({invalid_type_error: 'ID deve ser um número.'}),
});

export const createProdutosSchema = z.object({
  projeto_id:        z.number("O ID do projeto é obrigatório").int("O ID do projeto deve ser um número inteiro").positive("O ID do projeto deve ser um número positivo"),          // NOT NULL FK
  ...
  classe:            z.string().trim().max(100).optional(),// NULL
  ...
  deletedAt:         z.string().trim().nullable().optional(),           // date NULL (soft delete)
  deletedBy:         z.number().nullable().optional()                   // ID do usuário que deletou o produto (soft delete)
});
